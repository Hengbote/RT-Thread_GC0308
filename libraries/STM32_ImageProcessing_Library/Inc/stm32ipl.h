...
{
	stm32ipl_err_Ok                 =   0,	/*操作成功*/			/**< No error. */
	stm32ipl_err_Generic            =  -1,	/*通用错误*/			/**< Generic error. */
	stm32ipl_err_InvalidParameter   =  -2,	/*函数参数无效*/		/**< Function parameter is not valid. */
	stm32ipl_err_OutOfMemory        =  -3,	/*内存不足*/			/**< No memory is available. */
	stm32ipl_err_BadPointer         =  -4,	/*无效指针*/			/**< Invalid pointer. */
	stm32ipl_err_UnsupportedFormat  =  -5,	/*格式不支持*/			/**< Format is not supported. */
	stm32ipl_err_OpeningFile        =  -6,	/*打开文件时出错*/		/**< Error opening file. */
	stm32ipl_err_ClosingFile        =  -7,	/*关闭文件时出错*/		/**< Error closing file. */
	stm32ipl_err_ReadingFile        =  -8,	/*读取文件时出错*/		/**< Error reading file. */
	stm32ipl_err_WritingFile        =  -9,	/*写入文件时出错*/		/**< Error writing file. */
	stm32ipl_err_SeekingFile        = -10,	/*在文件中定位时出错*/	/**< Error seeking file. */
	stm32ipl_err_NotImplemented     = -11,	/*函数未实现*/			/**< Function is not implemented. */
	stm32ipl_err_OpNotCompleted     = -12,	/*操作未完成*/			/**< Operation was not completed. */
	stm32ipl_err_WrongSize          = -13,	/*尺寸错误*/			/**< Size is wrong. */
	stm32ipl_err_EmptyImage         = -14,	/*图像为空*/			/**< Image is empty. */
	stm32ipl_err_EmptyMatrix        = -15,	/*矩阵为空*/			/**< Matrix is empty. */
	stm32ipl_err_WrongMatrixDim     = -16,	/*矩阵维度错误*/		/**< Matrix has wrong dimension. */
	stm32ipl_err_ZeroMatrixDim      = -17,	/*矩阵维度为0*/			/**< Matrix has zero dimension. */
	stm32ipl_err_ReadingDatabase    = -18,	/*读取数据库时出错*/	/**< Error reading the database. */
	stm32ipl_err_WritingDatabase    = -19,	/*写入数据库时出错*/	/**< Error writing the database. */
	stm32ipl_err_UnsupportedMethod  = -20,	/*不支持的方法*/		/**< Method is not supported. */
	stm32ipl_err_NotAllowed         = -21,	/*操作不被允许*/		/**< Operation is not allowed. */
	stm32ipl_err_NotInPlaceFunction = -22,	/*函数不支持就地操作*/	/**< Function does not work in place. */
	stm32ipl_err_OpeningSource      = -23,	/*打开源时出错*/		/**< Error opening source. */
	stm32ipl_err_WrongROI           = -24,	/*感兴趣区域错误*/		/**< ROI is wrong. */
} stm32ipl_err_t;
...
{
	stm32ipl_if_binary = 1,		//二进制图像格式
	stm32ipl_if_grayscale = 2,	//灰度图像格式
	stm32ipl_if_rgb565 = 4,		//RGB565图像格式
	stm32ipl_if_rgb888 = 8,		//RGB888图像格式
} stm32ipl_if_t;
