...
{
	IMAGE_BPP_BINARY,       /* BPP = 0 */			/*二进制图像格式，每个像素可以取0-1的值。图像行会用零进行填充，并对齐到32位*/	/**< Binary image. Each pixel can assume 0-1 values. Image lines are padded with zeros and aligned to 32 bits. */
	IMAGE_BPP_GRAYSCALE,    /* BPP = 1 */			/*灰度图像格式，每个像素可以取0-255的值*/	/**< Grayscale image. Each pixel can assume values in the range [0, 255]. */
	IMAGE_BPP_RGB565,       /* BPP = 2 */			/*RGB565 颜色图像格式，每个像素由16位表示，其中 R 和 B 通道用5位表示，G 通道用6位表示*/	/**< Color image. Each pixel is represented with 16 bits; R and B channels are described with 5 bits, G with 6 bits. */
	IMAGE_BPP_BAYER,        /* BPP = 3 */			/*Bayer 格式，但在 STM32IPL 中未被使用*/	/**< Not used by STM32IPL. */
	IMAGE_BPP_RGB888,       /* BPP = 4 STM32IPL */	/*RGB888 颜色图像格式，每个像素由24位表示，其中每个 EGB 通道用8位表示*/	/**< Color image. Each pixel is represented with 24 bits, 8 bits for each EGB channel. */
	IMAGE_BPP_JPEG          /* BPP > 4 */			/*JPEG 格式，但在 STM32IPL 中未被使用*/	/**< Not used by STM32IPL. */
} image_bpp_t;
...
{
	int w;		/**< Width of the image (pixels). */	//图像的宽度
	int h;		/**< Height of the image (pixels). */	//图像的高度
	int bpp;	/**< Format of the image (actually contains image_bpp_t values). */	//图像的格式，实际上是以像素为单位的位数
	union	//这是一个联合体 实际上是同一个指针
	{
		uint8_t *pixels;	/**< Pointer to the pixels data. */	//指向图像像素数据的指针
		uint8_t *data;		/**< Pointer to the pixels data. */	//指向图像像素数据的指针
	};
} image_t;
