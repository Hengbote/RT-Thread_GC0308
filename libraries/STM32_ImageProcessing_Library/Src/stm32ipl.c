 ...
 */
uint32_t STM32Ipl_DataSize(uint32_t width, uint32_t height, image_bpp_t format)	//返回给定属性的图像需要存储的数据内存大小（以字节为单位）
{
 ...
 */
uint32_t STM32Ipl_ImageDataSize(const image_t *img)		//返回图像数据缓冲区大小（以字节为单位）
{
	return img ? STM32Ipl_DataSize(img->w, img->h, (image_bpp_t)img->bpp) : 0;	//判断img是否为空 如果不为空返回给定属性的图像需要存储的数据内存大小（以字节为单位）
}
 ...
 */
bool STM32Ipl_ImageFormatSupported(const image_t *img, uint32_t formats)	//检查图像格式是否支持
{
