 ...
 * @return		stm32ipl_err_Ok on success, error otherwise.
 */
stm32ipl_err_t STM32Ipl_Downscale(const image_t *src, image_t *dst, bool reversed)		//采用最近邻的方法 进行缩放
{
	int32_t dstW;	//目标图像(转换后)的宽度
	int32_t dstH;	//目标图像(转换后)的高度
	int32_t wRatio;	//宽度比例因子
	int32_t hRatio;	//高度比例因子

	STM32IPL_CHECK_VALID_IMAGE(src)					//检测源图像(转换前)是否为空
	STM32IPL_CHECK_VALID_IMAGE(dst)					//检测目标图像(转换后)是否为空
	STM32IPL_CHECK_FORMAT(src, STM32IPL_IF_ALL)		//检测源图像(转换前)格式是否是支持
	STM32IPL_CHECK_SAME_FORMAT(src, dst)			//检测源图像(转换前)和目标图像(转换后)格式是否相同

	if ((dst->w < 1) || (dst->h < 1))				//检测目标图像(转换后)大小设置是否正确
		return stm32ipl_err_InvalidParameter;		//函数参数无效

	dstW = dst->w;
	dstH = dst->h;

	wRatio = (int32_t) ((src->w << 16) / dst->w) + 1;	//保留精度避免浮点运算
	hRatio = (int32_t) ((src->h << 16) / dst->h) + 1;	//保留精度避免浮点运算
	...
	if (reversed) {
		switch (src->bpp) {
			...
			case IMAGE_BPP_RGB565:
				for (int32_t y = dstH - 1; y >= 0; y--) {
					uint16_t *srcRow = IMAGE_COMPUTE_RGB565_PIXEL_ROW_PTR(src, (y * hRatio) >> 16);	/*计算RGB565源图像(转换前)第y行位置的指针*/
					uint16_t *dstRow = IMAGE_COMPUTE_RGB565_PIXEL_ROW_PTR(dst, y);					/*计算RGB565目标图像(转换后)第y行位置的指针*/

					for (int x = dstW - 1; x >= 0; x--)
						IMAGE_PUT_RGB565_PIXEL_FAST(dstRow, x,										/*设置该位置RGB565像素值*/
								IMAGE_GET_RGB565_PIXEL_FAST(srcRow, (x * wRatio) >> 16));			/*获取该位置RGB565像素值*/
				}
			...
			default:
				return stm32ipl_err_UnsupportedFormat;		/*格式不支持*/
		}
	} else {
		switch (src->bpp) {
			case IMAGE_BPP_BINARY:
